...
test_that("prior & posterior data.table arguments work for 2^5 arg possibilities", {
  ...
  expect_no_error(plot_posterior(posterior_dt, true_param_values = true_vals, skip_stanfit_to_dt = TRUE))
  expect_no_error(plot_posterior(posterior_dt, prior_samples = prior_dt, skip_stanfit_to_dt = TRUE))
  expect_no_error(plot_posterior(posterior_dt, prior_samples = prior_dt, true_param_values = true_vals, skip_stanfit_to_dt = TRUE))
  expect_no_error(plot_posterior(posterior_dt, params_desired = c("m", "sigma"), skip_stanfit_to_dt = TRUE))
  expect_no_error(plot_posterior(posterior_dt, params_desired = c("m", "sigma"), true_param_values = true_vals, skip_stanfit_to_dt = TRUE))
  expect_no_error(plot_posterior(posterior_dt, params_desired = c("m", "sigma"), prior_samples = prior_dt, skip_stanfit_to_dt = TRUE))
  expect_no_error(plot_posterior(posterior_dt, params_desired = c("m", "sigma"), prior_samples = prior_dt, true_param_values = true_vals, skip_stanfit_to_dt = TRUE))
  expect_no_error(plot_posterior(posterior_dt, transform = list(sigma = log), skip_stanfit_to_dt = TRUE))
  expect_no_error(plot_posterior(posterior_dt, transform = list(sigma = log), true_param_values = true_vals, skip_stanfit_to_dt = TRUE))
  expect_no_error(plot_posterior(posterior_dt, transform = list(sigma = log), prior_samples = prior_dt, skip_stanfit_to_dt = TRUE))
  expect_no_error(plot_posterior(posterior_dt, transform = list(sigma = log), prior_samples = prior_dt, true_param_values = true_vals, skip_stanfit_to_dt = TRUE))
  expect_no_error(plot_posterior(posterior_dt, transform = list(sigma = log), params_desired = c("m", "sigma"), skip_stanfit_to_dt = TRUE))
  expect_no_error(plot_posterior(posterior_dt, transform = list(sigma = log), params_desired = c("m", "sigma"), true_param_values = true_vals, skip_stanfit_to_dt = TRUE))
  ...
  expect_no_error(plot_posterior(posterior_dt, transform = list(sigma = log), params_desired = c("m", "sigma"), prior_samples = prior_dt, true_param_values = true_vals, skip_stanfit_to_dt = TRUE))
  expect_no_error(plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), skip_stanfit_to_dt = TRUE))
  expect_no_error(plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), true_param_values = true_vals, skip_stanfit_to_dt = TRUE))
  expect_no_error(plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), prior_samples = prior_dt, skip_stanfit_to_dt = TRUE))
  expect_no_error(plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), prior_samples = prior_dt, true_param_values = true_vals, skip_stanfit_to_dt = TRUE))
  expect_no_error(plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), params_desired = c("m", "sigma"), skip_stanfit_to_dt = TRUE))
  expect_no_error(plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), params_desired = c("m", "sigma"), true_param_values = true_vals, skip_stanfit_to_dt = TRUE))
  ...
  expect_no_error(plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), params_desired = c("m", "sigma"), prior_samples = prior_dt, true_param_values = true_vals, skip_stanfit_to_dt = TRUE))
  expect_no_error(plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), transform = list(sigma = log), skip_stanfit_to_dt = TRUE))
  expect_no_error(plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), transform = list(sigma = log), true_param_values = true_vals, skip_stanfit_to_dt = TRUE))
  ...
  expect_no_error(plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), transform = list(sigma = log), prior_samples = prior_dt, true_param_values = true_vals, skip_stanfit_to_dt = TRUE))
  ...
  expect_no_error(plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), transform = list(sigma = log), params_desired = c("m", "sigma"), true_param_values = true_vals, skip_stanfit_to_dt = TRUE))
  ...
  expect_no_error(plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), transform = list(sigma = log), params_desired = c("m", "sigma"), prior_samples = prior_dt, true_param_values = true_vals, skip_stanfit_to_dt = TRUE))
})
...
if (do_manual_visual_testing) {
  ...
  plot_posterior(posterior_dt, true_param_values = true_vals, skip_stanfit_to_dt = TRUE)
  plot_posterior(posterior_dt, prior_samples = prior_dt, skip_stanfit_to_dt = TRUE)
  plot_posterior(posterior_dt, prior_samples = prior_dt, true_param_values = true_vals, skip_stanfit_to_dt = TRUE)
  plot_posterior(posterior_dt, params_desired = c("m", "sigma"), skip_stanfit_to_dt = TRUE)
  plot_posterior(posterior_dt, params_desired = c("m", "sigma"), true_param_values = true_vals, skip_stanfit_to_dt = TRUE)
  plot_posterior(posterior_dt, params_desired = c("m", "sigma"), prior_samples = prior_dt, skip_stanfit_to_dt = TRUE)
  plot_posterior(posterior_dt, params_desired = c("m", "sigma"), prior_samples = prior_dt, true_param_values = true_vals, skip_stanfit_to_dt = TRUE)
  plot_posterior(posterior_dt, transform = list(sigma = log), skip_stanfit_to_dt = TRUE)
  plot_posterior(posterior_dt, transform = list(sigma = log), true_param_values = true_vals, skip_stanfit_to_dt = TRUE)
  plot_posterior(posterior_dt, transform = list(sigma = log), prior_samples = prior_dt, skip_stanfit_to_dt = TRUE)
  plot_posterior(posterior_dt, transform = list(sigma = log), prior_samples = prior_dt, true_param_values = true_vals, skip_stanfit_to_dt = TRUE)
  plot_posterior(posterior_dt, transform = list(sigma = log), params_desired = c("m", "sigma"), skip_stanfit_to_dt = TRUE)
  plot_posterior(posterior_dt, transform = list(sigma = log), params_desired = c("m", "sigma"), true_param_values = true_vals, skip_stanfit_to_dt = TRUE)
  ...
  plot_posterior(posterior_dt, transform = list(sigma = log), params_desired = c("m", "sigma"), prior_samples = prior_dt, true_param_values = true_vals, skip_stanfit_to_dt = TRUE)
  plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), skip_stanfit_to_dt = TRUE)
  plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), true_param_values = true_vals, skip_stanfit_to_dt = TRUE)
  plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), prior_samples = prior_dt, skip_stanfit_to_dt = TRUE)
  plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), prior_samples = prior_dt, true_param_values = true_vals, skip_stanfit_to_dt = TRUE)
  plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), params_desired = c("m", "sigma"), skip_stanfit_to_dt = TRUE)
  plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), params_desired = c("m", "sigma"), true_param_values = true_vals, skip_stanfit_to_dt = TRUE)
  ...
  plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), params_desired = c("m", "sigma"), prior_samples = prior_dt, true_param_values = true_vals, skip_stanfit_to_dt = TRUE)
  plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), transform = list(sigma = log), skip_stanfit_to_dt = TRUE)
  plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), transform = list(sigma = log), true_param_values = true_vals, skip_stanfit_to_dt = TRUE)
  ...
  plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), transform = list(sigma = log), prior_samples = prior_dt, true_param_values = true_vals, skip_stanfit_to_dt = TRUE)
  ...
  plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), transform = list(sigma = log), params_desired = c("m", "sigma"), true_param_values = true_vals, skip_stanfit_to_dt = TRUE)
  ...
  plot_posterior(posterior_dt, labels = c(sigma = "log(sigma)"), transform = list(sigma = log), params_desired = c("m", "sigma"), prior_samples = prior_dt, true_param_values = true_vals, skip_stanfit_to_dt = TRUE)
